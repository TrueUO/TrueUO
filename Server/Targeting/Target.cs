	...
	public abstract class Target
	{
		...
		public void Invoke(Mobile from, object targeted)
		{
			...
			else
			{
				...
				else if (targeted is Item accessItem && !accessItem.IsAccessibleTo(from))
				{
					OnTargetNotAccessible(from, accessItem);
				}
				else if (targeted is Item item && !item.CheckTarget(from, this, item))
				{
					OnTargetUntargetable(from, item);
				}
