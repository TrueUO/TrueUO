...
namespace Server
{
	...
	public class Item : IEntity, IHued, IComparable<Item>, ISerializable, ISpawnable
	{
		...
		public byte GridLocation
		{
			...
			set
			{
				...
				{
					if (value > 0x7C || !((Container)Parent).IsFreePosition(value))
					{
		...
		public void MoveToWorld(Point3D location, Map map)
		{
			...
			if (m_Map != map)
			{
				...
				if (m_Map != null)
				{
					...
					{
						IPooledEnumerable<NetState> eable = m_Map.GetClientsInRange(oldLocation, Core.GlobalRadarRange - 4);
				...
				if (m_Map != null)
				{
					IPooledEnumerable<NetState> eable = m_Map.GetClientsInRange(m_Location, Core.GlobalRadarRange);
		...
		public virtual void AddItem(Item item)
		{
            ...
            if (item.m_Parent is Mobile mobileParent)
            {
                mobileParent.RemoveItem(item);
            }
            else if (item.m_Parent is Item itemParent)
            {
		...
		public void ProcessDelta()
		{
			...
			if (map != null && !Deleted)
			{
				...
				if ((flags & ItemDelta.Update) != 0)
				{
					...
					IPooledEnumerable<NetState> eable = map.GetClientsInRange(worldLoc, Core.GlobalMaxUpdateRange);
		...
		public void SendRemovePacket()
		{
			if (!Deleted && m_Map != null)
			{
				...
				IPooledEnumerable<NetState> eable = m_Map.GetClientsInRange(worldLoc, Core.GlobalRadarRange - 4);
		...
		public virtual void OnItemAdded(Item item)
		{
			if (m_Parent is Item itemParent)
			{
				itemParent.OnSubItemAdded(item);
			}
			else if (m_Parent is Mobile mobileParent)
			{
				mobileParent.OnSubItemAdded(item);
			}
		}
		...
		public virtual void OnItemRemoved(Item item)
		{
			if (m_Parent is Item itemParent)
			{
				itemParent.OnSubItemRemoved(item);
			}
			else if (m_Parent is Mobile mobileParent)
			{
				mobileParent.OnSubItemRemoved(item);
			}
		}
		...
		public virtual void OnItemBounceCleared(Item item)
		{
			if (m_Parent is Item itemParent)
			{
				itemParent.OnSubItemBounceCleared(item);
			}
			else if (m_Parent is Mobile mobileParent)
			{
				mobileParent.OnSubItemBounceCleared(item);
			}
		}
		...
		public virtual bool CheckTarget(Mobile from, Target targ, object targeted)
		{
			if (m_Parent is Item itemParent)
			{
				return itemParent.CheckTarget(from, targ, targeted);
			}

            if (m_Parent is Mobile mobileParent)
            {
                return mobileParent.CheckTarget(from, targ, targeted);
            }
		...
		{
			Packet p = GetStatusPacketFor(state);
		...
		public virtual bool IsAccessibleTo(Mobile check)
		{
			if (m_Parent is Item itemParent)
			{
				return itemParent.IsAccessibleTo(check);
			}
		...
		public virtual void OnItemUsed(Mobile from, Item item)
		{
			if (m_Parent is Item itemParent)
			{
				itemParent.OnItemUsed(from, item);
			}
			else if (m_Parent is Mobile mobileParent)
			{
				mobileParent.OnItemUsed(from, item);
			}
		}
		...
		public virtual bool CheckItemUse(Mobile from, Item item)
        {
            if (m_Parent is Item itemParent)
			{
				return itemParent.CheckItemUse(from, item);
			}

            if (m_Parent is Mobile mobileParent)
            {
                return mobileParent.CheckItemUse(from, item);
            }
		...
		public virtual void OnItemLifted(Mobile from, Item item)
		{
			if (m_Parent is Item itemParent)
			{
				itemParent.OnItemLifted(from, item);
			}
			else if (m_Parent is Mobile mobileParent)
			{
				mobileParent.OnItemLifted(from, item);
			}
		}
		...
		public virtual bool CheckLift(Mobile from, Item item, ref LRReason reject)
        {
            if (m_Parent is Item itemParent)
			{
				return itemParent.CheckLift(from, item, ref reject);
			}

            if (m_Parent is Mobile mobileParent)
            {
                return mobileParent.CheckLift(from, item, ref reject);
            }
	...
	public class ItemSocket
	{
		...
		{
			Type typeName = ScriptCompiler.FindTypeByName(reader.ReadString());
