...
namespace Server
{
    ...
    public class Mobile : IEntity, IHued, IComparable<Mobile>, ISerializable, ISpawnable, IDamageable
    {
        ...
        private int totalDamageAfterDeath;
        private readonly Dictionary<Mobile, int> m_DamagerRecord = new Dictionary<Mobile, int>();
        public Mobile GetRandomDamagerByDamage()
        {
            if (m_DamageEntries.Count == 0)
                return null;

            if (m_DamageEntries.Count == 1)
                return m_DamageEntries[0].Damager;

            if (totalDamageAfterDeath == 0)
            {
                foreach (DamageEntry kvp in m_DamageEntries)
                {
                    totalDamageAfterDeath += kvp.DamageGiven;
                    m_DamagerRecord.Add(kvp.Damager, totalDamageAfterDeath);
                }
            }
            if (totalDamageAfterDeath != 0)
            {
                int randomDamage = Utility.RandomMinMax(1, totalDamageAfterDeath);

                foreach (KeyValuePair<Mobile, int> kvp in m_DamagerRecord)
                {
                    if (kvp.Value >= randomDamage)
                    {
                        return kvp.Key;
                    }
                }
        ...
        public Mobile GetRandomDamager()
        {
            if (m_DamageEntries.Count == 0)
                return null;

            return m_DamageEntries[Utility.Random(m_DamageEntries.Count)]?.Damager;
        }
        ...
        */
        ...
        public virtual void ProcessDelta()
        {
            ...
            if (m.m_Map != null && (sendRemove || sendIncoming || sendPublicStats || sendHits || sendMoving || sendOPLUpdate || sendHair || sendFacialHair || sendHealthbarPoison || sendHealthbarYellow || sendFace))
            {
        ...
        {
            return p.m_X >= m_Location.m_X - range && p.m_X <= m_Location.m_X + range && p.m_Y >= m_Location.m_Y - range && p.m_Y <= m_Location.m_Y + range;
        }
        ...
        {
            return p.X >= m_Location.m_X - range && p.X <= m_Location.m_X + range && p.Y >= m_Location.m_Y - range && p.Y <= m_Location.m_Y + range;
        }
