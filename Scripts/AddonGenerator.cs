...
namespace Arya.Misc
{
    public class AddonGenerator
    {
        ...
        /// </summary>
        private static readonly string m_CustomOutputDirectory = null;
...
{complexnamecomponentscode}
	...
	public class {name}AddonDeed : BaseAddonDeed
	{
        ...
        private static void PickerCallback(Mobile from, Map map, Point3D start, Point3D end, object state)
        {
            ...
            var m_SimpleComponents = 0;
            var m_ComplexComponents = 0;
            var m_NamedComponents = 0;
            var m_TotalComponents = 0;
            ...
            if (getTiles)
            {
                for (var x = start.X; x <= end.X; x++)
                {
                    ...
                    {
#if RC2
                        var stlist = map.Tiles.GetStaticTiles(x, y, true);
                        var list = new List<StaticTile>();
                        var remove = new List<StaticTile>();
#else
            ...
            try
            {
                foreach (var o in en)
                {
                    ...
                    if (getItems)
                    {
                        ...
                        if (i is BaseAddon) // Not a good idea to add a BaseAddOn for obvious reasons
                        {
                            continue;
                        }

                        if (i.ItemID < 2 || i.ItemID > 36653) // This is not an Item within the normal artwork.. multi... etc.. Toss it
                        {
                            continue;
                        }

                        if (includeZRange && (i.Z < min || i.Z > max))
                        {
                            continue;
                        }
                        ...
                        if (includeItemRange && (i.ItemID < minItemID || i.ItemID > maxItemID))
                        {
                            continue;
                        }
            ...
            {
#if RC2
                var list = tiles[p] as List<StaticTile>;
#else
            ...
            var output = m_Template.Replace("{name}", name);
        ...
        {
            private readonly CEOIdentifyAddon m_IdentifyAddon;
        ...
        private class InternalGump : Gump
        {
            ...
            private void MakeGump()
            {
                ...
                string[] exportString = { "Statics", "Items", "Tiles" };
                for (var i = 0; i < 3; i++)
                {
            ...
            public override void OnResponse(NetState sender, RelayInfo info)
            {
                ...
                {
                    BoundingBoxPicker.Begin(sender.Mobile, PickerCallback, m_State);
                }
                else
                {
            ...
            private static bool Verify(Mobile from, object[] state)
            {
                ...
                string[] errors = {"Z Range Min", "Z Range Max","Static Min ID", "Static Max ID",
