...
namespace Server.Custom
{
    public class SearchImageCommand
    {
        ...
        public static void Initialize()
        {
            ...
            if (ImagesLoaded)
            {
                CommandSystem.Register("SearchImage", AccessLevel.GameMaster, SearchImage_OnCommand);
                CommandSystem.Register("GetImage", AccessLevel.GameMaster, SearchImage_OnCommand);
                CommandSystem.Register("FindImage", AccessLevel.GameMaster, SearchImage_OnCommand);
                CommandSystem.Register("BrowseImage", AccessLevel.GameMaster, BrowseImages_OnCommand);
                CommandSystem.Register("BrowseImages", AccessLevel.GameMaster, BrowseImages_OnCommand);
            }
        }
        ...
        public static void BrowseImages_OnCommand(CommandEventArgs e)
        {
            ...
            {
                e.Mobile.SendGump(new BrowseImageNamesGump(0));
            }
            else
            {
    ...
    {
        private int m_Index;
        private readonly string[] letters = { "a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m",
            ...
            : base(140, 70)
        {
            ...
            var list = SearchImageCommand.ImageNames;
            if (list != null && list.Count > 0)
            {
                ...
                {
                    string name = SearchImageCommand.ImageNames[q];
    ...
    public class ImageDetailsGump : Gump
    {
            ...
            : base(140, 70)
        {
            ...
            AddPage(1);
            AddLabel(30, 70, 0, $"Bridge: {id.Bridge.ToString()}");
            AddLabel(30, 90, 0, $"CalcHeight: {id.CalcHeight.ToString()}");
            AddLabel(30, 110, 0, $"Height: {id.Height.ToString()}");
            AddLabel(30, 130, 0, $"Impassable: {id.Impassable.ToString()}");
            AddLabel(30, 150, 0, $"Quality: {id.Quality.ToString()}");
            AddLabel(30, 170, 0, $"Quantity: {id.Quantity.ToString()}");
            AddLabel(30, 190, 0, $"Surface: {id.Surface.ToString()}");
            AddLabel(30, 210, 0, $"Value: {id.Value.ToString()}");
            AddLabel(30, 230, 0, $"Weight: {id.Weight.ToString()}");
            AddLabel(30, 250, 0, $"Flag Value: {((int) id.Flags).ToString()}");
    ...
    public class SearchImageGump : Gump
    {
        ...
        private readonly bool m_ShowImage;
            ...
            : base(0, 54)
        {
                ...
                if (list.Count > 0)
                {
                    ...
                    for (int q = index; q < index + num; q++)
                    {
                        try
                        {
                            ...
                            string name = TileData.ItemTable[list[q]].Name;
                        ...
                        catch
                        {
                            ...
                            AddHtml(x, 92, 60, 120, $"Unable to show Image ID {list[q].ToString()}.", false, false);
                        }
            ...
            AddLabel(20, 520, 380, $"{list.Count.ToString()} images found.");
        ...
        private readonly int[] m_SecretDoors = {
        ...
        private readonly int[] m_NormalDoors = {
        ...
        private readonly int[] m_TileFloors = {
        ...
        private readonly int[] m_WoodFloors = {
        ...
        private readonly int[] m_RusticFloors = {
        ...
        private readonly int[] m_TatamiFloors = {
        ...
        private readonly int[] m_OtherFloors = {
        ...
        private readonly int[] m_WoodWalls = {
        ...
        private readonly int[] m_StoneWalls = {
        ...
        private readonly int[] m_MarbleWalls = {
        ...
        private readonly int[] m_PlasterWalls = {
        ...
        private readonly int[] m_ElvenWalls = {
        ...
        private readonly int[] m_OtherWalls = {
        ...
        private readonly int[] m_RusticRoofs = {
        ...
        private readonly int[] m_CeramicRoofs = {
        ...
        private readonly int[] m_HardRoofs = {
        ...
        private readonly int[] m_WoodenRoofs = {
        ...
        private readonly int[] m_ElvenRoofs = {
        ...
        private readonly int[] m_NewAgeRoofs = {
        ...
        private readonly int[] m_MiscArches = {
        ...
        private readonly int[] m_VariousMisc = {
        ...
        private readonly int[] m_RoundedWalls = {
        ...
        private readonly int[] m_Battlements = {
        ...
        private readonly int[] m_MiscRoofs = {
        ...
        private readonly int[] m_MiscFences = {
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            else if (x >= 200000)
            {
                ...
                BoundingBoxPicker.Begin(m, TileBox_Callback, new TileState(subArgs));
            }
            else if (x >= 100000)
            {
                int itemID = x - 100000;
        ...
        private class TileState
        {
            ...
            private TileState(bool useFixedZ, int fixedZ, string[] args)
            {
        ...
        private class CreateItemTarget : Target
        {
            ...
            private readonly bool mShowImage;
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (target is StaticTarget staticTarget) location.Z -= TileData.ItemTable[staticTarget.ItemID & 0x3FFF].CalcHeight;
