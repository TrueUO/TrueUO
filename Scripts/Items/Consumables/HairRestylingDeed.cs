        ...
        private class InternalGump : Gump
        {
            ...
            */
                ...
                : base(50, 50)
            {
                ...
                for (int i = 1; i < HumanArray.Length; i++)
                {
                    AddHtmlLocalized(LayoutArray[i][2], LayoutArray[i][3], i == 1 ? 125 : 80, i == 1 ? 70 : 35, m_From.Female ? HumanArray[i][0] : HumanArray[i][1], false, false);
                    if (LayoutArray[i][4] != 0)
                    {
                        AddBackground(LayoutArray[i][0], LayoutArray[i][1], 50, 50, 0xA3C);
                        AddImage(LayoutArray[i][4], LayoutArray[i][5], m_From.Female ? HumanArray[i][4] : HumanArray[i][5]);
                    }
            ...
            public override void OnResponse(NetState sender, RelayInfo info)
            {
                ...
                if (m_From is PlayerMobile pm)
                {
                    ...
                    pm.HairItemID = pm.Female ? HumanArray[info.ButtonID][2] : HumanArray[info.ButtonID][3];
                    m_Deed.Delete();
                }
            }
        }
