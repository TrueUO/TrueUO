...
namespace Server.Mobiles
{
    public class AttendantHerald : PersonalAttendant
    {
        ...
        public override void OnMovement(Mobile m, Point3D oldLocation)
        {
            ...
            {
                if (HasControlOrder(OrderType.Follow) && m_NextYell < DateTime.UtcNow && m != ControlMaster && m_Announcement != null)
                {
                    m_Announcement.Say(this, m);
                    m_NextYell = DateTime.UtcNow + YellDelay + TimeSpan.FromSeconds(Utility.RandomMinMax(-2, 2));
                }
                else if (HasControlOrder(OrderType.Stay) && m_Greeting != null)
                {
        ...
        public override bool InGreetingMode(Mobile owner)
        {
            ...
            return m_House != null && m_House.IsOwner(owner) && HasControlOrder(OrderType.Stay);
        }
        ...
        private class OptionsGump : Gump
        {
                ...
                : base(200, 200)
            {
                ...
                if (herald.HasControlOrder(OrderType.Stay))
                {
            ...
            public override void OnResponse(NetState sender, RelayInfo info)
            {
                ...
                switch (info.ButtonID)
                {
                    ...
                    case 5:
                        {
                            if (m_Herald.HasControlOrder(OrderType.Follow))
                            {
                                BaseHouse house = BaseHouse.FindHouseAt(m_Herald);

                                if (house != null && house.IsOwner(m))
                                {
                                    m_Herald.AddControlOrder(OrderType.Stay);
                    ...
                    case 6:
                        {
                            if (m_Herald.HasControlOrder(OrderType.Stay))
                            {
                                m_Herald.AddControlOrder(OrderType.Follow);
