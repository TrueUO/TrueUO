    ...
    public static class TreasureMapInfo
    {
        ...
        public static void Fill(Mobile from, TreasureMapChest chest, TreasureMap tMap)
        {
                    ...
                    if (list.Length > 0)
                    {
                        ...
                        if (deco is SkullGnarledStaff || deco is GargishSkullGnarledStaff || deco is SkullLongsword || deco is GargishSkullLongsword)
                        {
                            ...
                            else
                            {
                                ...
                                RunicReforging.GenerateRandomItem(deco, from is PlayerMobile pm ? pm.RealLuck : from.Luck, min, max, chest.Map, from, ((int)level) / 2);
                            }
                        }
            ...
            foreach (Type type in GetRandomEquipment(package, facet, amount))
            {
                ...
                if (item != null)
                {
                    RunicReforging.GenerateRandomItem(item, from is PlayerMobile pm ? pm.RealLuck : from.Luck, min, max, chest.Map, from, ((int)level) / 2);
                    chest.DropItem(item);
                }
            }
