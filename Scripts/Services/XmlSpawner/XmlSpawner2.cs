    ...
    public class XmlSpawner : Item, ISpawner
    {
        ...
        public bool Spawn(bool smartspawn, byte loops)
        {
            if (m_SpawnObjects != null && m_SpawnObjects.Count > 0 && (m_proximityActivated || CanFreeSpawn) && TODInRange)
            {
                ...
                // Pick a spawn object to spawn and see if sequential spawning has been selected
                var SpawnIndex = m_SequentialSpawning >= 0 ? GetCurrentAvailableSequentialSpawnIndex(m_SequentialSpawning) : RandomAvailableSpawnIndex();
        ...
        private static bool HasTileSurface(Map map, int X, int Y, int Z)
        {
            ...
            foreach (StaticTile o in tiles) // go through the tiles and see if any are at the Z location
            {
                StaticTile i = o;
                        ...
                        if (SpawnObjectDetails.Length == 2)
                        {
                            ...
                            if (SpawnObjectDetails[0].Length > 0)
                            {
                                ...
                                if (SpawnObjectDetails[1].Length > 0)
                                {
                                    ...
                                    // Create the spawn object and store it in the array list
                                    SpawnObject so = new SpawnObject(SpawnObjectDetails[0], maxCount, subGroup, resetTime, resetTo, killsNeeded, restrictKills, clearAdvance, minD, maxD, spawnsPer, packRange);
