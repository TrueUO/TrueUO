    ...
    public class AuraOfNausea : AreaEffect
    {
        ...
        public override void DoEffect(BaseCreature creature, Mobile defender)
        {
            ...
            if (_Table.ContainsKey(defender))
            {
                ...
                _Table[defender] = creature.Controlled ? Timer.DelayCall(TimeSpan.FromSeconds(5), EndNausea, defender) : Timer.DelayCall(TimeSpan.FromSeconds(30), EndNausea, defender);
            }
            else
            {
            ...
            BuffInfo.AddBuff(defender, new BuffInfo(BuffIcon.AuraOfNausea, 1153792, 1153819, creature.Controlled ? TimeSpan.FromSeconds(5) : TimeSpan.FromSeconds(30), defender, "60\t60\t60\t5"));
        }
