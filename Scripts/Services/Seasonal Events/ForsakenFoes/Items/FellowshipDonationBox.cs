...
namespace Server.Items
{
    public class FellowshipDonationBox : BaseContainer
    {
        ...
        public bool Check(Mobile from, Item item)
        {
            ...
            from.SendLocalizedMessage(1159032, $"{Donations[from]}"); // The Fellowship thanks you for your donation. You have donated ~1_val~ worth of goods!

            if (Donations[from] >= 450000000)
            {
                from.SendLocalizedMessage(1152339, $"{Donations[from]}"); // A reward of ~1_ITEM~ has been placed in your backpack.
