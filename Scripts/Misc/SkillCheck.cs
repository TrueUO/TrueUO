    ...
    public class SkillCheck
    {
        ...
        public static void Gain(Mobile from, Skill skill, int toGain)
        {
            ...
            if (skill.Base < skill.Cap && skill.Lock == SkillLock.Up)
            {
                ...
                if (from is PlayerMobile mobile1 && skill.SkillName == mobile1.AcceleratedSkill &&
                    mobile1.AcceleratedStart > DateTime.UtcNow)
                {
                    // You are infused with intense energy. You are under the effects of an accelerated skillgain scroll.
                    mobile1.SendLocalizedMessage(1077956);
                ...
                else if (from is BaseCreature creature1 && !(creature1 is Engines.Despise.DespiseCreature) && (creature1.Controlled || creature1.Summoned))
                {
                    Mobile master = creature1.GetMaster();

                    if (master != null)
                    {
                        WhisperingSpell spell = SkillMasterySpell.GetSpell(master, typeof(WhisperingSpell)) as WhisperingSpell;

                        if (spell != null && master.InRange(creature1.Location, spell.PartyRange) && master.Map == creature1.Map &&
            ...
            if (from is PlayerMobile from1)
            {
                QuestHelper.CheckSkill(from1, skill);
            }
